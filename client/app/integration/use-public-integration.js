...
function useSaved({ contentApi, csrfToken, enabled, expire, markSaved, openArticle, onNavigate, onAskAboutArticle }) {
  ...
  const load = useCallback(async ({ cursor = null, targetPage = 1 } = {}) => {
    ...
    setError(null)
    try {
      const response = await contentApi.listSavedArticles({ limit: 20, ...(cursor ? { cursor } : {}) })
      const items = responseData(response, [])
      const nextMeta = responseMeta(response)
      setArticles(items)
      setMeta({ ...nextMeta, page: targetPage })
      setCursors((current) => {
        const next = current.slice(0, targetPage)
        next[targetPage - 1] = nextMeta.nextCursor ?? null
        return next
  ...
  useEffect(() => {
    if (!enabled || hasAttemptedRef.current) return undefined
    hasAttemptedRef.current = true
    const task = Promise.resolve().then(() => load({ cursor: null, targetPage: 1 }))
    void task
    return undefined
  }, [enabled, load])
  ...
  async function clear() {
    ...
    try {
      ...
      setCursors([null])
      failedRequestRef.current = { cursor: null, targetPage: 1 }
      setClearOpen(false)
      setState('ready')
    } catch (requestError) {
      expire(requestError)
  ...
  return {
    ...
    clearOpen,
    handlers: {
      onRetry: () => load(failedRequestRef.current),
      onPreviousPage: () => {
        const currentPage = Number(meta.page) || 1
        if (currentPage <= 1) return false
        return load({ cursor: currentPage > 2 ? cursors[currentPage - 3] : null, targetPage: currentPage - 1 })
      },
      onNextPage: () => {
        const currentPage = Number(meta.page) || 1
        if (!meta.hasNext) return false
        return load({ cursor: cursors[currentPage - 1] ?? null, targetPage: currentPage + 1 })
      },
      onUnsave: unsave,
      onOpenArticle: openArticle,
