...
function CreateSourceForm({ onSubmit, busy, error, onClose }) {
  ...
  const handleSubmit = async (event) => {
    ...
    await onSubmit(buildSourceCreateInput(form))
    if (!error && onClose) onClose()
  }

  return (
    <form className="source-form" onSubmit={handleSubmit} aria-describedby={error ? 'source-form-error' : undefined}>
      <div className="form-heading flex items-center justify-between">
        <h3>Tạo nguồn draft</h3>
        <span className="policy-chip">fail closed</span>
      </div>
      ...
      <div className="flex gap-2 mt-4">
        ...
        {onClose ? <button className="text-button" type="button" onClick={onClose}>Hủy</button> : null}
      </div>
    </form>
  )
}
...
export function SourceDetails({ source, handlers, busy, error, headingRef }) {
  ...
  // Cut ngắn ID hiển thị (6 ký tự đầu...4 ký tự cuối)
  const shortId = source.id ? `${source.id.slice(0, 6)}...${source.id.slice(-4)}` : ''

  return (
    <section className="source-details" aria-labelledby="source-detail-title">
      <div className="source-title-row">
        <div>
          <span className="eyebrow" title={source.id}>{source.sourceKey} · <code style={{ opacity: 0.7 }}>ID: {shortId}</code></span>
          <h2 id="source-detail-title" ref={headingRef} tabIndex="-1">{source.name}</h2>
        </div>
        <div className="source-badges">
          <span>{source.operationalStatus}</span>
          <span>{source.licenseStatus}</span>
        </div>
      </div>
      <dl className="policy-rail">
        <div><dt>Policy</dt><dd>v{source.policyVersion}</dd></div>
        <div><dt>Reconciliation</dt><dd>{source.reconciliation.status} · v{source.reconciliation.requiredPolicyVersion}</dd></div>
        <div><dt>Technical</dt><dd>{source.technicalCheck.status}</dd></div>
        <div><dt>LLM scope</dt><dd>{source.llmInputScope}</dd></div>
      </dl>
  ...
  return (
    <section className="source-registry" aria-labelledby="source-registry-title">
      <div className="operator-header">
        <div>
          <span className="eyebrow">ADMIN · SOURCE REGISTRY</span>
          <h1 id="source-registry-title">Nguồn, quyền và trạng thái vận hành.</h1>
        </div>
        <div style={{ display: 'flex', gap: '8px' }}>
          <button className="primary-button" type="button" onClick={() => setShowAddTab(true)}>+ Thêm nguồn</button>
          <button className="text-button" type="button" onClick={handlers.onReload} disabled={busy}>Làm mới</button>
        </div>
      </div>
      ...
      {/* Modal / Tab Thêm Nguồn */}
      {showAddTab ? (
        <div className="source-modal-overlay" style={{ background: 'rgba(0,0,0,0.5)', padding: '20px', borderRadius: '8px', marginBottom: '20px' }}>
          <CreateSourceForm onSubmit={handlers.onCreate} busy={busy} error={error} onClose={() => setShowAddTab(false)} />
        </div>
      ) : null}
      ...
      {state === 'ready' ? (
        <div className="source-workspace">
          <aside className="source-list" aria-label="Danh sách nguồn">
            {sources.length === 0 ? <p>Chưa có nguồn nào. Bấm nút "+ Thêm nguồn" phía trên để tạo nguồn đầu tiên.</p> : sources.map((item) => (
              <button type="button" key={item.id} className={selected?.id === item.id ? 'selected' : ''} aria-pressed={selected?.id === item.id} onClick={() => handlers.onSelect(item)}>
                <strong>{item.name}</strong>
                <span>{item.connectorType} · {item.operationalStatus}</span>
                <small>policy v{item.policyVersion}</small>
              </button>
            ))}
          </aside>
          <div className="source-editor">
            {selected ? <SourceDetails source={selected} handlers={handlers} busy={busy} error={error} headingRef={detailHeadingRef} /> : <div className="source-state"><p>Chọn một nguồn để xem policy và trạng thái.</p></div>}
          </div>
        </div>
      ) : null}
    </section>
