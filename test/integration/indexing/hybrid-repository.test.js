...
function article(id, overrides = {}) {
  return {
    ...
    leadMedia: null, leadMediaStatus: 'none', embeddingStatus: 'ready', embeddingModel: 'embedding-model-v1', embeddingDimensions: 3, embeddingArtifactCompatibilityId: 'embedding-compat-v1', embeddingVersion: 7,
    embedding: Array(3).fill(0), _currentSource: source, _isSaved: [], ...overrides,
  }
}
...
describe('Step 9 Mongo hybrid retrieval repository', () => {
  it('unions text and compatible semantic candidates, ranks cosine and keeps HN community discovery', async () => {
    const text = article('507f1f77bcf86cd799439011', { _textScore: 4, embedding: [0, 0, 0] })
    const communitySource = { ...source, name: 'Hacker News', authorityTier: 'community-signal' }
    const semantic = article('507f1f77bcf86cd799439012', { titleOriginal: 'Community signal', authorityTier: 'community-signal', evidenceEligible: false, _currentSource: communitySource, embedding: [1, 0, 0] })
    ...
    const result = await repository.searchVisibleArticles({
      ...
      queryEmbedding: { model: 'embedding-model-v1', dimensions: 3, version: 7, artifactCompatibilityId: 'embedding-compat-v1', embedding: [1, 0, 0] },
    })
  ...
  it('returns the text page with an explicit fallback when no compatible vectors exist', async () => {
    ...
    const result = await repository.searchVisibleArticles({
      userId: USER_ID, q: 'chip AI', mode: 'hybrid', limit: 20,
      queryEmbedding: { model: 'embedding-model-v1', dimensions: 3, version: 7, artifactCompatibilityId: 'embedding-compat-v1', embedding: Array(3).fill(0.01) },
    })
