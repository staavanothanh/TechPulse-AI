...
describe('local Q&A CLI', () => {
  it('parses bounded one-shot options, timeout bounds, and supports help without requiring a question', () => {
    ...
    expect(() => parseQaCliArgs(['--question', 'x', '--timeout-ms', '99'])).toThrow(/between 100 and 300000/i)
    expect(() => parseQaCliArgs(['--question', 'x', '--timeout-ms', '300001'])).toThrow(/between 100 and 300000/i)
    expect(() => parseQaCliArgs(['--question', 'x', '--timeout-ms', '1.5'])).toThrow(/between 100 and 300000/i)
    ...
    let unknownError
    try { parseQaCliArgs(['--question', 'x', '--unknown=secret-argv-value']) } catch (error) { unknownError = error }
  ...
  it('resolves direct session credentials and prefers environment credentials over flags', () => {
    expect(resolveQaCliAuth({
      options: parseQaCliArgs(['--question', 'Câu hỏi hợp lệ', '--session-token', SESSION_TOKEN, '--csrf-token', 'flag-csrf-token-abcdefghijklmnopqrstuvwxyz-123456']),
  ...
  it('rejects CSRF tokens outside the inclusive 32..256 boundary before any request', () => {
    const options = parseQaCliArgs(['--question', 'Câu hỏi hợp lệ', '--session-token', SESSION_TOKEN])
    for (const token of ['x'.repeat(31), 'x'.repeat(257)]) {
      let error
      try { resolveQaCliAuth({ options, environment: { QA_CSRF_TOKEN: token } }) } catch (caught) { error = caught }
      expect(error).toMatchObject({ status: 403, code: 'csrf_invalid' })
    }
    expect(resolveQaCliAuth({ options, environment: { QA_CSRF_TOKEN: 'x'.repeat(32) } }).csrfToken).toHaveLength(32)
    expect(resolveQaCliAuth({ options, environment: { QA_CSRF_TOKEN: 'x'.repeat(256) } }).csrfToken).toHaveLength(256)
  })
  ...
  it('uses the default timeout and aborts a pending request with a safe timeout error', async () => {
    ...
    try {
      ...
      expect(requestInit.signal.aborted).toBe(false)
      const timedOut = expect(pending).rejects.toMatchObject({ status: 503, code: 'service_unavailable' })
  ...
  it('uses QA_TIMEOUT_MS when no timeout flag is supplied', async () => {
    ...
    try {
      ...
      const pending = runQaCli({
        options: parseQaCliArgs(['--question', 'Câu hỏi hợp lệ', '--topic', 'AI']),
        environment: { QA_SESSION_TOKEN: SESSION_TOKEN, QA_CSRF_TOKEN: CSRF_TOKEN, QA_TIMEOUT_MS: '100' },
        fetchImpl,
      })
  ...
  it('rejects an oversized successful response instead of emitting provider data', async () => {
    const fetchImpl = vi.fn(async () => jsonResponse({ data: ANSWER.data, providerPayload: 'oversized-provider-payload' }, 200, {
  ...
  it('logs in through the existing auth route in memory, then submits the answer without printing credentials', async () => {
    const fetchImpl = vi.fn()
      .mockResolvedValueOnce(jsonResponse({ data: { user: { id: 'user-1' }, csrfToken: LOGIN_CSRF_TOKEN } }, 200, {
  ...
  it('requires a bounded login CSRF token and a session cookie before answer handoff', async () => {
    const options = parseQaCliArgs(['--question', 'Câu hỏi hợp lệ', '--topic', 'AI', '--email', 'user@example.com', '--password', 'secret'])
    ...
    for (const csrfToken of ['x'.repeat(31), 'x'.repeat(257)]) {
      const { fetchImpl, pending } = runLogin({ data: { user: { id: 'user-1' }, csrfToken } }, { 'Set-Cookie': cookie })
    ...
    const missingCookie = runLogin({ data: { user: { id: 'user-1' }, csrfToken: LOGIN_CSRF_TOKEN } })
    await expect(missingCookie.pending).rejects.toMatchObject({ status: 502, code: 'internal_error' })
  ...
  it('accepts valid answered/refused data and rejects bare or extra-key 2xx envelopes', async () => {
    ...
    const run = (body) => runQaCli({ options, environment, fetchImpl: vi.fn(async () => jsonResponse(body)) })
    await expect(run(ANSWER)).resolves.toMatchObject({ ok: true, status: 200, body: ANSWER })

    const refused = { data: { ...ANSWER.data, id: 'answer-refused-envelope', status: 'refused', paragraphs: [], citations: [], refusalReason: 'insufficient-evidence' } }
    ...
    expect(result).toMatchObject({ ok: false, status: 502, body: { error: { code: 'internal_error' } } })
  ...
  it('accepts a full session cookie and rejects incomplete session or login credentials without network access', () => {
    expect(resolveQaCliAuth({
      options: parseQaCliArgs(['--question', 'Câu hỏi hợp lệ', '--session-cookie', `__Host-techpulse_session=${encodeURIComponent(SESSION_TOKEN)}; Path=/`, '--csrf-token', CSRF_TOKEN]),
    })).toEqual({ mode: 'session', cookie: `__Host-techpulse_session=${SESSION_TOKEN}`, csrfToken: CSRF_TOKEN })
    expect(() => resolveQaCliAuth({
  ...
  it('returns refused answers unchanged and leaves malformed idempotency keys to the web route', async () => {
    const refused = { data: { ...ANSWER.data, id: 'answer-refused', status: 'refused', paragraphs: [], citations: [], refusalReason: 'insufficient-evidence' } }
    const fetchImpl = vi.fn(async () => jsonResponse(refused))
    const result = await runQaCli({
      options: parseQaCliArgs(['--question', 'Câu hỏi hợp lệ', '--topic', 'AI', '--idempotency-key', 'bad']),
      environment: { QA_SESSION_TOKEN: SESSION_TOKEN, QA_CSRF_TOKEN: CSRF_TOKEN },
      fetchImpl,
    })
  ...
  it('maps transport and malformed login failures to safe machine-readable errors', async () => {
    ...
    const loginFailure = await runQaCli({
      options: parseQaCliArgs(['--question', 'Câu hỏi hợp lệ', '--topic', 'AI', '--email', 'user@example.com', '--password', 'secret']),
      fetchImpl: vi.fn(async () => jsonResponse({ error: { code: 'unauthorized', message: 'Email or password is invalid', requestId: 'req-login' } }, 401)),
    })
    expect(loginFailure).toEqual({ ok: false, status: 401, stage: 'login', body: { error: { code: 'unauthorized', message: 'Email or password is invalid', requestId: 'req-login' } } })

    const missingCookie = vi.fn(async () => jsonResponse({ data: { user: { id: 'user-1' }, csrfToken: LOGIN_CSRF_TOKEN } }))
    await expect(runQaCli({
      options: parseQaCliArgs(['--question', 'Câu hỏi hợp lệ', '--topic', 'AI', '--email', 'user@example.com', '--password', 'secret']),
  ...
  it('rejects an invalid base URL and reports CLI validation failures without exposing option values', async () => {
    await expect(runQaCli({
      options: parseQaCliArgs(['--question', 'Câu hỏi hợp lệ', '--topic', 'AI', '--base-url', 'https://example.com']),
      environment: { QA_SESSION_TOKEN: SESSION_TOKEN, QA_CSRF_TOKEN: CSRF_TOKEN },
      fetchImpl: vi.fn(),
    })).rejects.toMatchObject({ status: 400, code: 'bad_request' })
    await expect(runQaCli({
      options: parseQaCliArgs(['--question', 'Câu hỏi hợp lệ', '--topic', 'AI', '--base-url', 'https://user:password@example.com']),
      environment: { QA_SESSION_TOKEN: SESSION_TOKEN, QA_CSRF_TOKEN: CSRF_TOKEN },
      fetchImpl: vi.fn(),
    })).rejects.toMatchObject({ status: 400, code: 'bad_request' })

    const errorLog = vi.fn()
    const result = await main(['--question', 'x', '--topic', 'AI', '--session-token', SESSION_TOKEN, '--csrf-token', CSRF_TOKEN], { errorLog })
    expect(result).toMatchObject({ ok: false, status: 422, body: { error: { code: 'validation_error' } } })
  ...
  it('returns help through the runner without attempting authentication or HTTP', async () => {
    const fetchImpl = vi.fn()
    await expect(runQaCli({ options: { help: true }, fetchImpl })).resolves.toEqual({ ok: true, help: true })
    expect(fetchImpl).not.toHaveBeenCalled()
  })
  ...
  it('prints HTTP errors to stderr while keeping the web error envelope unchanged', async () => {
    ...
    const result = await main(['--question', 'Câu hỏi hợp lệ', '--topic', 'AI'], {
      environment: { QA_SESSION_TOKEN: SESSION_TOKEN, QA_CSRF_TOKEN: CSRF_TOKEN },
      fetchImpl: vi.fn(async () => jsonResponse({ error: { code: 'service_unavailable', message: 'Try later', requestId: 'req-503' } }, 503)),
      errorLog,
    })
  ...
  it('redacts unexpected local exceptions to a stable internal error envelope', async () => {
    ...
    const result = await main(['--question', 'Câu hỏi hợp lệ', '--topic', 'AI', '--email', 'user@example.com', '--password', secret], {
