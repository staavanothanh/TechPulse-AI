...
function liveRepository(repository) {
  return { ...repository, assertActiveSessionForUser: vi.fn(async () => true) }
}
...
describe('admin governance service', () => {
  ...
  it('replaces persisted article diagnostic text with an allowlisted safe error', async () => {
    ...
    const service = createAdminGovernanceService({ repository: liveRepository({ findAdminArticle: vi.fn(async () => article) }) })
  ...
  it('rejects non-admin and unavailable repository safely', async () => {
    const service = createAdminGovernanceService({ repository: liveRepository({ getOverview: vi.fn() }) })
  ...
  it('fails closed when the repository cannot verify or rejects the admin session', async () => {
    const unavailable = createAdminGovernanceService({ repository: { getOverview: vi.fn() } })
    await expect(unavailable.getAdminOverview({ auth: adminAuth })).rejects.toMatchObject({ status: 503, code: 'service_unavailable' })

    const revoked = createAdminGovernanceService({ repository: {
      assertActiveSessionForUser: vi.fn(async () => false),
      getOverview: vi.fn(),
    } })
    await expect(revoked.getAdminOverview({ auth: adminAuth })).rejects.toMatchObject({ status: 401, code: 'unauthorized' })
  })
})
