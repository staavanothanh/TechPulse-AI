...
describe('Step 9 indexing Mongo repository documents', () => {
  ...
  it('fans each ingested article into independent, deterministic summary and embedding jobs', () => {
    ...
    const first = buildIngestionArtifactJobs({ source, article, now: createdAt })
    const replay = buildIngestionArtifactJobs({ source, article, now: createdAt })
    ...
    expect(first.find(({ task }) => task === 'embedding')).toEqual(expect.objectContaining({ targetEmbeddingVersion: 1 }))
