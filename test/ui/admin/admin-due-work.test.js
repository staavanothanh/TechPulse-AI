...
describe('admin due-work controls', () => {
  ...
  it('renders the manual run action and aggregate counters for the three queues', () => {
    ...
    expect(html).toContain('Chạy một lượt hàng đợi')
    expect(html).toContain('Kết quả lượt chạy giới hạn gần nhất')
    expect(html).toContain('Thu thập dữ liệu')
    expect(html).toContain('Chỉ mục hóa')
    ...
    expect(html).toContain('Hoãn lại')
