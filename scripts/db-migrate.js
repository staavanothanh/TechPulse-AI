...
if (!['auth-core', 'sources', 'durable-jobs', 'articles', 'indexing-jobs', 'indexing-drain-performance', 'provider-routing-v2', 'chat-sessions', 'qa-evidence-fence', 'summary-detail-v1', 'governance'].includes(target)) {
  console.error(
    'Supported migration targets: auth-core, sources, durable-jobs, articles, indexing-jobs, indexing-drain-performance, provider-routing-v2, chat-sessions, qa-evidence-fence, summary-detail-v1, governance',
  )
  process.exitCode = 2
} else {
  try {
    ...
    const plan = dryRun
      ...
      : await (async () => {
          ...
          const plan = await runMigration({ db: context.db, ...(target === 'summary-detail-v1' ? { writerMode: summaryDetailWriterMode } : {}) })
