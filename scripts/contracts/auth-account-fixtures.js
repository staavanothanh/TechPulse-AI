...
function fixtureService() {
  ...
  return {
    ...
    async changePassword({ currentPassword }) {
      if (currentPassword === 'wrong') throw new AuthError(403, 'forbidden', 'Current password is incorrect')
      return authResult()
    },
...
export async function runAuthAccountContractFixtures({ document } = {}) {
  ...
  try {
    ...
    await request('changePassword', '/api/v1/me/password', { method: 'POST', headers: { ...jsonHeaders, Cookie: userCookie, 'X-CSRF-Token': CSRF_TOKEN }, body: JSON.stringify({ currentPassword: VALID_CREDENTIAL, newPassword: 'brand-new-password' }) }, 200)
    await request('changePassword', '/api/v1/me/password', { method: 'POST', headers: { ...jsonHeaders, Cookie: userCookie, 'X-CSRF-Token': CSRF_TOKEN }, body: JSON.stringify({ currentPassword: 'wrong', newPassword: 'brand-new-password' }) }, 403)
    await request('changePassword', '/api/v1/me/password', { method: 'POST', headers: { ...jsonHeaders, Cookie: userCookie, 'X-CSRF-Token': CSRF_TOKEN }, body: JSON.stringify({ newPassword: 'short' }) }, 422)
    await request('changePassword', '/api/v1/me/password', { method: 'POST', headers: { ...jsonHeaders, 'X-CSRF-Token': CSRF_TOKEN }, body: JSON.stringify({ newPassword: 'brand-new-password' }) }, 401)
