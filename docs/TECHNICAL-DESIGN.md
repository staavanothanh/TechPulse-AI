...
IP-HMAC field-unset và cron lifecycle-event purge dùng client/credential riêng qua `MONGODB_MAINTENANCE_URI_ENV`, fixed predicates và batch<=100. URI/identity phải khác runtime; thiếu config làm `purge-audit-ip-hmac` và `purge-cron-lifecycle-events` unavailable và maintenance-retention release gate fail, không được fallback sang runtime credential. Full audit event purge, offline checkpoint key custody, signed retention manifest và restore verifier thuộc recovery track hậu MVP; không là điều kiện phục vụ MVP.
...
| Maintenance credential thiếu                     | Audit IP-HMAC và cron lifecycle retention báo unavailable | Chỉ `purge-audit-ip-hmac` và `purge-cron-lifecycle-events` bị disable và maintenance-retention release gate fail; core runtime/fixed task khác không dùng runtime credential để thay thế |
