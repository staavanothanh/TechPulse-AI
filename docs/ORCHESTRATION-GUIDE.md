...
- **Prerequisite:** chỉ chạy một step khi dependency trong blueprint có handoff/verification evidence; Step 1 chỉ bắt đầu sau Plan-of-Record baseline v1.5.
- **Authority:** prompt dưới đây là entrypoint, không thay thế tasks/exit criteria trong blueprint hoặc OpenAPI/PRD/Data Model.
- **Ownership collision:** Step 3 sở hữu source marker; Step 4 sở hữu queue registry/generic runner/leases/ingestion; Step 7 chỉ tạo article/intent, không materialize marker; Step 9 đăng ký indexing + sole marker materialization/checkpoint; Step 11 đăng ký deletion + governance. Không sửa migration/file step khác nếu chưa handoff.
...
```bash
/ecc:orchestrate custom "ecc:tdd-guide,ecc:build-error-resolver,ecc:code-reviewer" "[Plan: docs/plans/techpulse-ai-mvp.md#step-1] Scaffold React/Vite JS/JSX + Express/Vercel; pin tooling và contract/test/eval/db/build scripts. Acceptance: OpenAPI completeness TP-M01 và kill-tests pass cho HttpsUrl/media host, Source attribution/reconciliation states, operation reasonCode, ingestion expected version, unavailable citation/takedown completion; generated JS imports, bundle sạch. Out of scope: DB, auth, business UI, connector/provider thật."
```
...
```bash
/ecc:orchestrate custom "ecc:tdd-guide,ecc:code-reviewer,ecc:security-reviewer" "[Plan: docs/plans/techpulse-ai-mvp.md#step-3] Implement source CRUD/policy/re-review, connector matrix, policyVersion, marker validators và atomic reasonCode audit. Acceptance: contradictory policy/connector, Source attribution true+missing|null|empty, invalid reconciliation terminal states và IP-literal media host bị reject; re-review pause+version+pending marker. Out of scope: Network check thật, job materialization, automatic license interpretation."
```
...
```bash
/ecc:orchestrate custom "ecc:tdd-guide,ecc:database-reviewer,ecc:code-reviewer,ecc:security-reviewer" "[Plan: docs/plans/techpulse-ai-mvp.md#step-4] Implement queue registry, canonical resource keys, no-TTL high-water, exact heartbeat, linked/same-request recovery adapters và reserved queue-local fairness; register ingestion only. Safe-fetch pin validated public IP per hop. Acceptance: cron/manual same source contend; expired lease không resurrect; sustained due queues progress; unregistered no-query/zero summary; DNS rebinding chặn. Out of scope: Connector parse, article persistence."
```
...
```bash
/ecc:orchestrate custom "ecc:tdd-guide,ecc:database-reviewer,ecc:code-reviewer,ecc:security-reviewer" "[Plan: docs/plans/techpulse-ai-mvp.md#step-9] Register indexing adapter; implement one-task jobs/admin HTTP+UI, expected policy fenced artifacts và BGE-M3 fallback. Sole-owner materialize source marker with exact policy/status/cursor CAS and versioned fan-out identity. Acceptance: pending-provider output discard; N→N+1 worker cannot mutate marker; indexing progresses under backlog; top-5/text fallback pass; no full text. Out of scope: Claim citation, fine-tuning, Atlas Vector Search."
```
...
```bash
/ecc:orchestrate custom "ecc:tdd-guide,ecc:code-reviewer,ecc:security-reviewer" "[Plan: docs/plans/techpulse-ai-mvp.md#step-10] Implement bounded grounded chat và eval. Capture user/session lifecycle before provider; final chat/quota append CAS active user + exact sessionVersion + current article state. Add available/unavailable citation union. Acceptance: delayed provider after user/article transition persists nothing; unavailable with URL/title rejected; citation/refusal gates pass. Out of scope: Tools, live web, claim-level citation, multilingual output."
```
...
```bash
/ecc:orchestrate custom "ecc:tdd-guide,ecc:database-reviewer,ecc:code-reviewer,ecc:security-reviewer" "[Plan: docs/plans/techpulse-ai-mvp.md#step-11] Register deletion same-request recovery; complete admin/governance. Takedown converts historical citations unavailable and requires chat-redaction evidence; deletion increments sessionVersion, preserves flags on retry; user audit actor opaque. Acceptance: fake delayed Q&A after takedown/deletion recreates nothing; sustained 3-queue progress; reason/status mismatch rejected; audit/PII safe. Out of scope: Superadmin, MFA, SSO, partial approval."
```
...
```bash
/ecc:orchestrate custom "ecc:tdd-guide,ecc:e2e-runner,ecc:build-error-resolver,ecc:security-reviewer" "[Plan: docs/plans/techpulse-ai-mvp.md#step-12] Run full contract/eval/security/E2E, deploy và restore rehearsal. Acceptance: canonical keys/heartbeat, linked-vs-same recovery, queue fairness, ingestion/reconciliation policy races, delayed Q&A deletion/takedown, Source/reason/citation conditionals, media browser boundary, DNS-pinned server fetch, citation gates và secret/fulltext/binary scans all have evidence. Out of scope: Production SLA, unrestricted launch, post-MVP."
```
