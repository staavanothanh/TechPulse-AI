...
- **Prerequisite:** chỉ chạy một step khi dependency trong blueprint có handoff/verification evidence; Step 1 chỉ bắt đầu sau Plan-of-Record repair v1.4.
- **Authority:** prompt dưới đây là entrypoint, không thay thế tasks/exit criteria trong blueprint hoặc OpenAPI/PRD/Data Model.
- **Ownership collision:** Step 3 sở hữu source reconciliation marker; Step 4 sở hữu generic runner + persistent leases + ingestion jobs; Step 9 sở hữu indexing jobs và materialize marker; Step 11 sở hữu content takedown/account deletion/audit completion. Không sửa migration/file của step khác nếu chưa ghi handoff.
...
```bash
/ecc:orchestrate custom "ecc:tdd-guide,ecc:build-error-resolver,ecc:code-reviewer" "[Plan: docs/plans/techpulse-ai-mvp.md#step-1] Scaffold React/Vite JS/JSX + Express local/Vercel app; pin tooling và tạo mọi contract/test/eval/db/build script cùng health endpoint. Acceptance: clean build/test; OpenAPI completeness và kill-tests cho HttpsUrl, reasonCode, attribution, cron aggregate, expected policy version pass; generated JS artifacts import được; bundle không lộ env. Out of scope: DB, auth, business UI, connector/provider thật."
```
...
```bash
/ecc:orchestrate custom "ecc:tdd-guide,ecc:code-reviewer,ecc:security-reviewer" "[Plan: docs/plans/techpulse-ai-mvp.md#step-3] Implement source CRUD/policy/re-review, connector matrix, policyVersion, durable reconciliation marker và atomic reasonCode audit. Acceptance: contradictory policy/connector và attribution true+null bị reject; unclear rights metadata-only; re-review pause+version+pending marker trong source; audit không free-form/snapshot. Out of scope: Network check thật, indexing job/materialization, automatic license interpretation."
```
...
```bash
/ecc:orchestrate custom "ecc:tdd-guide,ecc:database-reviewer,ecc:code-reviewer,ecc:security-reviewer" "[Plan: docs/plans/techpulse-ai-mvp.md#step-4] Implement two-phase due-work, ingestion jobs, idempotency, persistent no-TTL lease high-water, crash recovery và cron aggregate. Safe-fetch phải validate all A/AAAA, reject mixed/mapped/private, pin socket và manual redirect. Acceptance: linked retry đúng một lần; stale commit fail; cron 3 queue summaries; DNS rebinding bị chặn. Out of scope: Connector parse, article persistence."
```
...
```bash
/ecc:orchestrate custom "ecc:tdd-guide,ecc:code-reviewer,ecc:security-reviewer" "[Plan: docs/plans/techpulse-ai-mvp.md#step-8] Implement feed/detail/saved/cursor/text search bằng generated JS client; render approved media và external HttpsUrl với safe rel. Acceptance: login-to-source hoạt động khi AI tắt; hidden/blocked/media ngoài policy không leak; javascript/data/file/credential URL bị reject; image fallback và responses validate contract. Out of scope: Semantic ranking, AI Q&A, personalization/recommendation."
```
...
```bash
/ecc:orchestrate custom "ecc:tdd-guide,ecc:database-reviewer,ecc:code-reviewer,ecc:security-reviewer" "[Plan: docs/plans/techpulse-ai-mvp.md#step-9] Implement one-task indexing jobs với expectedSourcePolicyVersion, fenced artifact commit, marker materialization, JS provider adapters và BGE-M3 hybrid/text fallback. Acceptance: policy đổi khi provider pending làm output discard; stale job không ghi; marker checkpoint hoàn tất; outage không phá text search; top-5 đạt; không persist full text. Out of scope: Claim citation, fine-tuning, Atlas Vector Search."
```
...
```bash
/ecc:orchestrate custom "ecc:tdd-guide,ecc:database-reviewer,ecc:code-reviewer,ecc:security-reviewer" "[Plan: docs/plans/techpulse-ai-mvp.md#step-11] Complete admin ops, all-or-nothing takedown, automatic deletion và safe reasonCode audit. Acceptance: case text/PII không vào list/audit; sessions revoked rồi saved/chat/quota/identity cleanup trước completed; direct mutation+audit atomic; retry không restore identity; unrelated reasonCode bị reject; user bị chặn admin. Out of scope: Superadmin, MFA, SSO, partial approval, prompt/model editor."
```
...
```bash
/ecc:orchestrate custom "ecc:tdd-guide,ecc:e2e-runner,ecc:build-error-resolver,ecc:security-reviewer" "[Plan: docs/plans/techpulse-ai-mvp.md#step-12] Run full contract/eval/security/E2E, deploy Vercel/Mongo, verify generic cron recovery, 3–5 sessions và restore rehearsal. Acceptance: citation gates đạt; crash/high-water/stale-policy, DNS rebinding, HTTPS URL, reasonCode audit, takedown/deletion pass; DB/log/bundle sạch; mọi PRD gate có evidence. Out of scope: Production SLA, unrestricted launch, commercial/post-MVP features."
```
