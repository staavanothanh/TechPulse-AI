...
```text
Reference chain: "ecc:tdd-guide,ecc:e2e-runner,ecc:build-error-resolver,ecc:security-reviewer" "[Plan: docs/plans/techpulse-ai-mvp.md#step-12] Run MVP contract/integration/E2E/eval/security/build gates, deploy smoke and prepare the demo. Block release unless ADR-0013 remediation proves model fallback, full provider-domain outage fallback, terminal no-fallback classes, privacy equivalence, max-attempt cap and embedding compatibility/text degradation. Verify Atlas mirrored capability probes, account-deletion inline recovery index, secret/fulltext/binary scans. Backup/restore rehearsal, sidecar signing and restore serving are post-MVP and not this release gate. Out of scope: production SLA/unrestricted launch."
```
